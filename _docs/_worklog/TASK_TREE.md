...
> - 루트 노드의 모든 자식이 완료(`[x]` 또는 `[-]`)되어 루트 자체가 완결되면, 다음 작업 시작 전에 사용자에게 커밋을 제안한다. 사용자가 커밋을 승인하면 같은 커밋에 완결된 루트 노드를 트리에서 제거하고 [DONE.md](DONE.md)에 이력을 기록한다. 트리에는 활성/대기 작업만 남긴다 - 이력은 git log + DONE.md가 SSOT(single source of truth). 사용자가 커밋을 보류하면 즉시 제거하지 않고 다음 라운드까지 보존한다.
