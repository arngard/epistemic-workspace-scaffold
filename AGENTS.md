...
- 대화: 한국어, 존댓말.
- 비속어·욕설·모욕·차별적 표현은 응답·문서·commit message·PR 본문·댓글 어디에도 사용하지 않는다. 사용자가 사용하더라도 AI는 정중한 톤을 유지한다 — 사용자 어휘를 모방하면 AI 발화의 신뢰성·전문성이 떨어지고, 누적 문서·이력에 비속어가 영구화되면 사후 협업·검토 비용이 커진다. 사용자 발화를 그대로 인용해야 하는 경우(예: 인용·정정 대상 명시)는 따옴표로 인용 표시한다.
