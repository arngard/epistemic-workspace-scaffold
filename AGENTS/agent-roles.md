...
형식 점검(front matter 필수 필드, 폴더 디스크립터 부재, 폴더 직속 md 수 임계, 링크 정합, 파일명 이식성, 문서 유형별 분량 임계, 시간성 혼합 패턴, STATUS 서술, TASK_TREE 속성 키)은 [tools/audit.py](tools/audit.py)로 이관되었다 (cf. [tools.md](tools.md) "기계 감사 스크립트"). 감사자는 스크립트를 실행해 결과를 보고에 첨부하고, 그 형식 항목들을 손으로 재검사하지 않는다. 감사자 본체는 의미 판단과 git 맥락 검출에 집중한다.
