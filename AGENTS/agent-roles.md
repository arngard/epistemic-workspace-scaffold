...
2. 잔여 형식 항목 (스크립트 미담당)
   ...
   - "스캐폴드 상태" 마커와 실제 내용의 부합 - 채워졌는데 마커가 남았거나, 마커 없이 플레이스홀더만 남은 경우 (디스크립터와 대상의 불일치는 스크립트 소관)
...
- 새 세션 시작 시 트리거 조건을 충족하면 착수 전 호출 (cf. [../AGENTS.md](../AGENTS.md) "세션 시작"). 임계 조건의 SSOT는 STATUS 레지스트리의 주기/트리거 셀이다.
  - 판정: `check_semantic_audit.py`로 한다 (cf. [tool-environment.md](tool-environment.md)). 기준은 본 레포 자신의 클린 커밋 이후 새 커밋이 있고 마지막 수행이 임계 시간을 초과했는가이며, 그 커밋이 없으면 그 자체로 충족이다(최초 1회). 클린 커밋은 trailer를 줄 전체로 갖고 상류 머지로 흘러들지 않은 것만 인정한다 - 본문 언급과 상류 감사 이력을 기준점으로 오인하지 않는다.
  - 기준점 갱신: 호출 후 STATUS의 해당 행을 새 시각으로 갱신하고, 그 갱신과 발견 사항 정리(수정 또는 TASK_TREE 등재)를 담은 커밋에 trailer `Semantic-Audit: clean`을 남긴다. 작업 브랜치 경유로 main에 머지되면 머지 커밋에도 같은 trailer를 남긴다(기준점이 머지 이후를 가리키도록).
- 긴 세션 종료 전, 큰 변경 후
- 사용자가 정합성 의심 시
