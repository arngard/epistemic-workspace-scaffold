...
- 새 세션 시작 시 STATUS 레지스트리의 에피스테믹 의미 감사 행이 그 행의 주기/트리거 조건을 충족하면 본 작업 시작 전에 자동 호출 (cf. [../AGENTS.md](../AGENTS.md) "세션 시작" 3번). 임계 조건의 SSOT는 STATUS 레지스트리의 주기/트리거 셀이다.
  - 판정 절차: `git log -1 --grep "Semantic-Audit: clean"`으로 마지막 감사 클린 커밋을 찾는다. 그 커밋 이후 새 커밋이 존재하고 마지막 수행 시각이 임계 시간을 초과했으면 충족이다. 클린 커밋이 이력에 없으면 그 자체로 충족이다(최초 1회).
  - 기준점 갱신: 메인 에이전트가 호출 후 STATUS 레지스트리의 해당 행을 새 시각으로 갱신하고, 그 갱신과 발견 사항 정리(수정 또는 TASK_TREE 등재)를 담은 커밋 메시지에 trailer `Semantic-Audit: clean`을 남긴다. 이 갱신이 작업 브랜치 경유로 main에 머지되면 머지 커밋에도 같은 trailer를 남긴다(기준점이 머지 이후를 가리키도록).
- 긴 세션 종료 전, 큰 변경 후
- 사용자가 정합성 의심 시
