...
- 새 세션 시작 시 STATUS 레지스트리의 에피스테믹 의미 감사 행이 그 행의 주기/트리거 조건을 초과했으면 본 작업 시작 전에 자동 호출 (cf. [../AGENTS.md](../AGENTS.md) "세션 시작" 3번). 임계 수치의 SSOT는 STATUS 레지스트리의 주기/트리거 셀이다. 메인 에이전트가 호출 후 STATUS 레지스트리의 해당 행을 새 시각으로 갱신한다.
- 긴 세션 종료 전, 큰 변경 후
- 사용자가 정합성 의심 시
