...
2. 잔여 형식 항목 (스크립트 미담당)
   ...
   - 현재적 문서의 근거 보존 예외로 남긴 근거 요지가 경위 서사로 비대해지지 않았는가 (cf. [conventions.md](conventions.md) "시간성 혼합 금지")
3. git 맥락 검출 (스크립트 미담당, git 비의존 원칙상 audit.py 밖)
   ...
   - `date closed` 히스토리 문서의 본문 변경(동결 위반 - 후속 문서 링크 추가와 정합 유지 수정은 허용, cf. [conventions.md](conventions.md) "날짜 프리픽스의 의미와 종결 신호")
...
- 새 세션 시작 시 STATUS 레지스트리의 에피스테믹 의미 감사 행의 마지막 수행이 12시간 초과면 본 작업 시작 전에 자동 호출 (cf. [../AGENTS.md](../AGENTS.md) "새 세션 시작 시" 3번). 메인 에이전트가 호출 후 STATUS 레지스트리의 해당 행을 새 시각으로 갱신한다.
- 긴 세션 종료 전, 큰 변경 후
- 사용자가 정합성 의심 시
