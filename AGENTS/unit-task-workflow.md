...
- Labels: 변경 종류 라벨(chore·docs·feature·fix 등) + 영역 라벨(레포별 도메인에 따라). 라벨 풀이 비어 있으면 본 PR이 첫 라벨을 만들 수도 있다 — 향후 분류 비용 회수를 위해 작은 라벨이라도 박는다.
...
AI가 PR 본문이나 댓글·리뷰 reply를 작성할 때는 시그니처를 명시한다. 형식:
...
근거: AI는 사용자의 토큰·계정으로 GitHub에 작성한다. 시그니처가 없으면 외부 관찰자(다른 협업자·리뷰 봇·사후 본인)가 사용자의 직접 행동과 AI 자동 행동을 식별할 수 없어 의사 소통에 혼란을 일으킨다. 누가 무엇을 결정·작성했는지가 흐려지면 이후 정정·검토·인용 비용이 커진다.

도구별 적용:

- PR 본문: 일부 AI(Claude Code 등)는 자동으로 footer에 박는다. 자동 박힘 여부를 확인하고, 없으면 명시 추가.
- 댓글·리뷰 코멘트 reply: 자동 박힘 없는 경우가 많아 명시 추가 의무. 본문 마지막 한 줄로 단다.
