...
6. **서브모듈 commit point 재귀 정합화** (해당하는 경우): 본 레포가 다른 레포의 서브모듈로 등록돼 있다면, 그 상위 레포에서 서브모듈 pointer를 본 단위 작업 종료 시점의 main HEAD로 갱신한다 — `cd <상위 레포> && git add <서브모듈 경로> && git commit -m "..."`. 본 정합화는 재귀적이다 — 상위 레포가 다시 더 상위 레포의 서브모듈이면 거기까지 같은 절차를 반복한다. 트리의 모든 commit point가 정합 상태에 도달해야 단위 작업이 진짜 끝. 본 레포 입장에서는 자기가 등재된 곳만 챙기면 충분하지만, 본 레포가 다시 다른 자식 변경을 흡수하는 상위 노드라면 자식 단위 작업이 같은 원칙으로 본 레포까지 정렬을 올린다 — 양방향이 아니라 "본인이 변경되면 본인을 가리키는 상위까지 위로 흐른다"의 단방향 재귀. 부모 레포에 변경 노트 누적 문서(예: `UPGRADING.md`)가 있다면 자식 hash 추기까지 같은 단위에 묶는다. 본 단계가 누락되면 부모 레포가 옛 자식 commit을 가리키는 상태로 부정합 — 자식 sync는 끝나도 부모는 끝나지 않은 상태가 된다. 검증: 정합화 트리의 모든 노드에서 `git submodule status`의 `+` 표시 없음 + `git diff --submodule` 비어 있음. 본 레포가 어디에 등재됐는지는 본 레포 안에서 직접 보이지 않으므로, 부모 레포 자체 문서(예: README의 "자식 sync 라운드 종료" 같은 절차)에서 본 단계의 구체 시점·도구·문서 형식을 별도 관리한다.
